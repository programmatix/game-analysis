...
async function resolveCardImageSource({ card, imageSrc, face }, options = {}) {
  ...
  const fallbackBaseUrl = options.fallbackImageBaseUrl ? String(options.fallbackImageBaseUrl).trim() : '';
  ...
  const primary = face === 'back' ? images.back : images.front;
  const resolvedPrimary = typeof primary === 'string' ? primary.trim() : '';
  if (resolvedPrimary) return resolveImageUrl(resolvedPrimary, baseUrl, { label });

  if (fallbackBaseUrl) {
    const fallback = resolveImageUrl(`/bundles/cards/${encodeURIComponent(String(card?.code || '').trim())}.png`, fallbackBaseUrl, { label });
    return fallback;
  }
...
async function ensureCardImage({ card, imageSrc, face }, cacheDir, options = {}) {
  ...
  const identifier = sanitizeFileName(card?.fullName || card?.name || key);
  const faceSuffix = face ? `-${sanitizeFileName(face)}` : '';
  const fileName = `${identifier || 'card'}-${key}${faceSuffix}${ext}`;
  ...
  if (!response.ok) {
    const label = card?.fullName || card?.name || card?.code || basename || url;
